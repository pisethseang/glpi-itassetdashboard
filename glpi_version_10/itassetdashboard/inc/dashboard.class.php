...
class PluginItassetdashboardDashboard extends CommonGLPI {
    ...
    static function getComputers($filters = []) {
        global $DB;
        $result = $DB->query(self::getComputerQuery($filters));
        $rows = [];
        if ($result) {
    ...
    static function getSoftware($filters = []) {
        global $DB;
        $result = $DB->query(self::getSoftwareQuery($filters));
        $rows = [];
    ...
    static function getSoftwareStats() {
        ...
        // Also surfaces software on computers where the version end date has passed
        $today = date('Y-m-d');
        $r = $DB->query("
            SELECT
                s.name                              AS software,
                ...
                COALESCE(sv.end_of_support, '')     AS end_of_support,
                CASE
                    WHEN sv.end_of_support IS NOT NULL
                     AND sv.end_of_support != '0000-00-00'
                     AND sv.end_of_support < '$today'  THEN 'EOL'
                    ELSE 'No Licence'
                END AS vuln_type
            FROM glpi_items_softwareversions isv
            INNER JOIN glpi_softwareversions sv ON sv.id = isv.softwareversions_id
            ...
            WHERE isv.itemtype = 'Computer'
              AND sl.id IS NULL
            GROUP BY s.id, sv.id, s.name, sv.name, sv.end_of_support
    ...
    static function getSoftwareFilterOptions() {
        ...
        $r = $DB->query("SELECT DISTINCT COALESCE(sm.name,'') AS val
            FROM glpi_softwares s
            LEFT JOIN glpi_manufacturers sm ON sm.id = s.manufacturers_id
        ...
        $r = $DB->query("SELECT DISTINCT COALESCE(uc.name,'') AS val
            FROM glpi_computers c
            LEFT JOIN glpi_users u ON c.users_id = u.id
            LEFT JOIN glpi_usercategories uc ON u.usercategories_id = uc.id
        ...
        $r = $DB->query("SELECT DISTINCT s.name AS val
            FROM glpi_softwares s
        ...
        $r = $DB->query("SELECT DISTINCT c.name AS val
            FROM glpi_computers c
        ...
        // Software categories
        $r = $DB->query("SELECT DISTINCT sc.name AS val
            FROM glpi_softwares s
            LEFT JOIN glpi_softwarecategories sc ON sc.id = s.softwarecategories_id
